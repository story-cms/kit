...
- Header action buttons (`AppLayout` `#actions`) should use `primary` or `secondary` only; a lone button must be `primary`. Destructive is allowed.
- **Violations to fix before merge** (see checklist below):
  - `resources-index.vue` — lone Create Resource button is `secondary` (should be `primary`)
  - `resources-edit.vue` — Cancel is `tertiary` beside primary Save/Create (should be `secondary`)
  - `story-index.vue` — Edit is `tertiary` (should be `primary` when alone, `secondary` when Publish is visible)
  - `app-layout-header.story.vue` — Glass header Cancel is `tertiary`; Draft page lone Save is `secondary`

## Header action button rule — violations to fix

Rule: `StudioButton` in `AppLayout` `#actions` must be `primary` or `secondary`. Single button → `primary`. Destructive OK.

| Fixed | Component | Histoire | Issue | Expected |
|-------|-----------|----------|-------|----------|
| [ ] | `resources-index.vue` | [Resources index page](../../src/frontend/resources/resources-index.story.vue) | Create Resource uses `secondary` | Lone CTA → `primary` |
| [ ] | `resources-edit.vue` | [Resource edit](../../src/frontend/resources/resources-edit.story.vue) | Cancel uses `tertiary` | Paired with primary → `secondary` |
| [ ] | `story-index.vue` | [Story Index](../../src/frontend/stories/story-index.story.vue) | Edit uses `tertiary` | `primary` when published (Edit only); `secondary` when Edit + Publish |
| [ ] | `app-layout-header.story.vue` | [App Layout Header](../../src/frontend/shared/app-layout-header.story.vue) | Story examples contradict rule | Glass header: Cancel → `secondary`; Draft page: Save → `primary` |

**Already correct (spot-check):** `users-index.vue`, `invitations-index.vue`, `settings-index.vue`, `languages-edit.vue`, `story-create.vue`, `story-gallery.vue`, `story-edit.vue`, `chapter-preview.vue`.

**Out of scope:** modal footers, in-page buttons, legacy `IconButton`/`LabelButton` headers (`stream-index.vue`, `audience-index.vue`, `journa-story-edit.vue`).
...
| Language option label | shared | `language-option-label.vue` | [Language option label](../../src/frontend/shared/language-option-label.story.vue) |
...
| Nav rail language selector | shared | `nav-rail-language-selector.vue` | [Nav rail language selector](../../src/frontend/shared/nav-rail-language-selector.story.vue) |
...
| [ ] | App Layout Header | shared | `app-layout-header.vue` | [App Layout Header](../../src/frontend/shared/app-layout-header.story.vue) | Glass header / Draft page story variants follow the rule? |
...
| [ ] | Resources index page | resources | `resources-index.vue` | [Resources index page](../../src/frontend/resources/resources-index.story.vue) | Header Create Resource: is it primary (not secondary)? |
...
| [ ] | Story Index | stories | `story-index.vue` | [Story Index](../../src/frontend/stories/story-index.story.vue) | Published: Edit primary only? Unpublished: Edit secondary + Publish primary? |
