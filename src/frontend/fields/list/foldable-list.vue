<template>
  <ul v-for="(_listItem, index) in listItems" :key="index" class="row-[span_100] mb-8 grid grid-rows-subgrid">
    <li :class="[
      ...
      <template v-if="!isEmpty(index)">
        ...
        <button v-if="isExpanded(index) && !isReadOnly" type="button" class="absolute bottom-0 left-2 cursor-pointer rounded bg-white px-1.5 py-2 shadow-sm" @click="toggle(index)">
        ...
        <div class="relative flex items-center justify-between">
          ...
          <button type="button" class="z-[1] inline-flex items-center rounded-full border border-gray-300 bg-white px-4 py-1.5 text-sm font-medium leading-5 text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500" :disabled="isReadOnly" :class="{ 'opacity-0': isReadOnly }" @click.prevent="toggle(index)">
            <icon v-if="isExpanded(index)" name="chevron-down" class="icon mr-1" aria-hidden="true" />
          ...
          <button v-if="canMutate" type="button" class="z-[1] flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border bg-white text-gray-500" @click="emit('removeSet', index)">
        ...
        <ul v-if="isExpanded(index)" :class="[
          'row-[span_100] grid grid-rows-subgrid bg-white',
          { 'ml-8': !isReadOnly },
        ]">
          <li v-for="(item, i) in fields" :key="item.name + `${i.toString()}`" class="grid grid-rows-subgrid">
            <component :is="widgets.picker(item.widget)" :class="{
              'rounded bg-white': item.widget != 'list',
              'rounded bg-white p-8': isIsland(item.widget),
            }" :field="item" :is-read-only="props.isReadOnly" :root-path="`${fieldPath}.${index.toString()}`" :is-nested="true" />
          </li>
        </ul>
      </template>
    </li>
