<template>
  ...
  >
    ...
    <div class="mt-[2px] pt-1 sm:col-span-2 sm:mt-0">
      <div class="relative">
        <input
          v-if="!isReadOnly && showInput && pills.length === 0"
          ref="inputRef"
          :value="inputValue"
          type="text"
          autocomplete="off"
        ...
        >
          <span
            class="flex w-full flex-wrap items-center gap-2 text-base sm:text-sm/6"
          ...
          >
            ...
            <slot name="input">
              ...
              <input
                ...
                autocomplete="off"
                :name="field.label"
                class="inline-block w-auto min-w-[8ch] flex-1 border-0 bg-white px-0 py-1 text-sm font-normal leading-5 text-gray-900 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-white"
                @input="handleInputEvent"
                @keydown.enter.stop="handleEnterKey"
                @keydown.arrow-down.prevent="handleArrowDown"
                @keydown.arrow-up.prevent="handleArrowUp"
